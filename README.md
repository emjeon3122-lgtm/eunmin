...
### 청첩장/부고장 URL·사진 자동 채우기

신청서 4단계에서 모바일 청첩장/부고장 URL이나 사진(최대 2장)으로 배송 정보를 자동으로
채워주는 기능입니다. 업체마다 청첩장 화면 구조가 전부 달라 업체별 파서를 유지하는 건
현실적이지 않아, 사람이 눈으로 읽듯 **Claude 비전 모델**이 읽고 필요한 필드만 구조화해
돌려주는 방식입니다(`apps/api/src/invitation-parser`).

**기본값은 Mock이라 아무것도 외부로 나가지 않습니다.** 청첩장에는 이름·연락처 등
개인정보가 들어있어 외부 전송에 사내 검토가 필요하므로, 검토 후에만 켜세요:
...
채워지는 값과 처리 방식이 필드마다 다릅니다:

- **수령인 이름 / 상세주소 / 도착희망일시** — 자유 입력 칸이라 읽어낸 값을 그대로 채웁니다.
- **배송주소** — 읽어낸 값을 그대로 쓰지 않습니다. 주소가 틀리면 배송 자체가 실패하므로,
  그 값을 **검색어로 넣은 우편번호 검색 팝업**을 띄워 직원이 공식 주소를 클릭 한 번으로
  확정하게 합니다(다시 타이핑할 필요는 없습니다).

확실하지 않은 항목은 모델이 지어내지 않고 비워두도록 했고(빈 값은 기존 입력을 덮어쓰지
않습니다), 링크를 읽지 못하거나 추출에 실패해도 빈 결과만 돌려주고 신청 흐름은 막지
않습니다 — 원래 선택 입력입니다. 청첩장 업체가 외부 접근을 막거나 화면을 JavaScript로
그리는 경우가 흔해 **링크보다 사진이 더 안정적입니다.**
