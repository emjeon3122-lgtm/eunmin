...
1. **토큰 링크(기본, 항상 사용)** — 알림톡 버튼으로 열리는 `/vendor/status/[token]`에서
   업로드하면 토큰 자체가 주문을 특정하므로 매칭 문제가 없습니다.
2. **카톡 대화방 사진(보조 안전망)** — 꽃집이 링크 대신 채널 대화방에 사진만 올리는
   경우를 위해 `POST /api/webhooks/kakao-inbound-photo`가 그 꽃집의 "접수 확인됨" 주문
   중 정확히 1건일 때만 자동 매칭합니다. 0건이거나 2건 이상이면 절대 추측하지 않고
   관리자 알림 + `/admin/requests`의 "미매칭 사진" 패널에서 수동으로 연결합니다. 이
   경로도 실제 CPaaS가 채널 대화방 메시지를 웹훅으로 넘겨줘야 동작하므로, 마찬가지로
   실제 계약 전까지는 구조만 갖춰둔 상태입니다.
