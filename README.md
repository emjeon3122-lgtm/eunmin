...
임직원 신청서를 5개 섹션 다단계 폼으로 개편하는 작업을 1단계(DB 스키마 + 핵심 백엔드
API)부터 진행 중입니다. 이번 단계에서 반영된 것: 파트너 검증 게이팅, 고객사(현재/잠재
고객)·본인 구분과 그에 따른 비용코드/발송사유/계약정보 입력, 결혼 신랑측·신부측, 개업·
승진 동양란·서양란 구분, 주문자 본인 연락처, 청첩장 URL/OCR 자동 채우기(Mock), 첨부파일
형식·용량 검증. **다단계 폼 UI(프론트엔드)는 다음 단계에서 이어서 작업합니다** — 지금은
기존 한 페이지 폼(`/requests/new`)이 새 필드를 아직 반영하지 않은 채로 남아있어 백엔드
API와 화면이 일시적으로 어긋나 있습니다.
